...
describe('lebarGores — tekanan menentukan tebal', () => {
  it('tekanan lebih besar menghasilkan garis lebih tebal', () => {
    const ringan = SuntingHalamanComponent.lebarGores(4, 0.1);
    const sedang = SuntingHalamanComponent.lebarGores(4, 0.5);
    const berat = SuntingHalamanComponent.lebarGores(4, 1);
  ...
  it('tidak pernah nol, dan tidak pernah lebih dari dua kali tebal dasar', () => {
    ...
    for (const t of [-5, 0, 0.001, 0.5, 1, 9, NaN]) {
      const l = SuntingHalamanComponent.lebarGores(4, t as number);
  ...
  it('naik lebih cepat di tekanan rendah daripada di tekanan tinggi', () => {
    ...
    const a = SuntingHalamanComponent.lebarGores(4, 0.2)
            - SuntingHalamanComponent.lebarGores(4, 0.05);
    const b = SuntingHalamanComponent.lebarGores(4, 0.95)
            - SuntingHalamanComponent.lebarGores(4, 0.8);
...
describe('tekanan — nilai pengganti', () => {
  it('tetikus dan jari (tanpa tekanan) memakai 0,5', () => {
    expect(SuntingHalamanComponent.tekanan({ pressure: 0 })).toBe(0.5);
    expect(SuntingHalamanComponent.tekanan({} as any)).toBe(0.5);
  });

  it('tekanan pena dipakai apa adanya, dipotong di 1', () => {
    expect(SuntingHalamanComponent.tekanan({ pressure: 0.17 })).toBeCloseTo(0.17);
    expect(SuntingHalamanComponent.tekanan({ pressure: 3 })).toBe(1);
  });
});
