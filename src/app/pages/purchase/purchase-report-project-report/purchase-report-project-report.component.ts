...
export class PurchaseReportProjectReportComponent implements OnInit {
  ...
  loadReport() {
    ...
    this.apiService.get(`purchases/report/project/${projectName}`, {}).subscribe({
      next: (data: any) => {
        this.data = data;
        this.loadPie();
      ...
      error: (err) => {
        ...
        this.snackBar.open('Failed to load project report', 'Close', { duration: 3000 });
      }
    });
  }
  ...
  loadPie() {
    ...
    // Blue shades palette
    const blueShades = [
      '#001F4D', '#003380', '#0047B3', '#005CFF', '#007BFF',
      '#1A8CFF', '#339DFF', '#4DB0FF', '#66C2FF', '#80D4FF',
      '#99E6FF', '#B3F8FF', '#CCFBFF', '#E6FDFF', '#F2FEFF'
    ];

    const backgroundColors = sortedLabels.map((_, i) => blueShades[i % blueShades.length]);
  ...
  buildTypeTree() {
    ...
    const processItems = (items: any[], type: 'purchase' | 'reimbursement' | 'draft') => {
      items.forEach(item => {
        ...
        let totalValue = 0;
        if (type === 'purchase') {
          totalValue = item.dpp + (item.ppn * item.dpp) / 100 + item.pbbkb + item.otherValue;
  ...
  get totalPurchase(): number {
    ...
    else return this.data.purchases.reduce((a: any, b: any) => {
  ...
  get totalReimbursements(): number {
    if(this.data == null) return 0;
    else return this.data.reimbursements.reduce((a: any, b: any) => {
      return a + b.amount;
    }, 0);
  }
  ...
  get totalDrafts(): number {
    if(this.data == null) return 0;
    else return this.data.purchase_drafts.reduce((a: any, b: any) => {
  ...
  get dpp(): number {
    if(this.data == null) return 0;
      ...
      const reimbursements = this.data.reimbursements.reduce((a: any, b: any) => {
        return a + b.amount;
      },0);

      const purchaseDrafts = this.data.purchase_drafts.reduce((a: any, b: any) => {
        return a + b.dpp;
      },0);
